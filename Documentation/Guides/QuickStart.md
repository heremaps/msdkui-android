...
- Include the HERE Mobile SDK (Premium, 3.8 or newer) and the HERE Mobile SDK UI Kit: Copy the `HERE-sdk.aar` and the `MSDKUILib.aar` to _app/libs_ (if not done already, [build the MSDKUILib](ContributionGuide.md#building-the-here-ui-kit-library) via Gradle: `./gradlew clean :MSDKUILib:assembleRelease`).
