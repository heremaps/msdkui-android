...
- Include HERE Mobile SDK (Premium, 3.8+) and the HERE Mobile SDK UI Kit: Copy the `HERE-sdk.aar` and the `MSDKUILib.aar` to _app/libs_ (if not done already, [build the MSDKUILib](ContributionGuide.md#building-the-here-ui-kit-library-and-the-api-reference) via Gradle: `./gradlew clean :MSDKUILib:assembleRelease`).
- Open the project in Android Studio.
- Add your credentials to `AndroidManifest.xml` file. Make sure that the credentials match the package name. You may need to refactor the package name of the app to match the one you are registered to:
```xml
<meta-data
...
- Set a proper package name, make sure that it matches the one used for your HERE credentials.
...
>**Note:** Since we later on want to integrate the HERE SDK, please, make sure to set the same package name as on _developer.here.com_ when you acquired your HERE credentials. For the example, we have used _com.here.msdkui.example_. Please, adapt it to match your own package name.
...
Since HERE Mobile SDK UI Kit uses Lambda expressions, _Java 8_ is required, so please add the following to the `Android`-closure:
...
To enable quick Javadoc reference within your Android Studio environment, scroll down to the _External Libraries_ section, right-click on `MSDKUILib:@aar`, and then select _Library Properties_. Click the `+`-button and locate `MSDKUILib-javadoc.jar`.
