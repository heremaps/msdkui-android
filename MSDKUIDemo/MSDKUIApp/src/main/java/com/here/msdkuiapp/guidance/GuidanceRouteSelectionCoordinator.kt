...
class GuidanceRouteSelectionCoordinator(private val context: Context, fragmentManager: FragmentManager) :
        ...
        GuidanceRoutePreviewFragment.Listener {
    ...
    override fun onWaypointSelected(entry: WaypointEntry) {
        ...
        val fragment = addFragment(R.id.guidance_selection_bottom_container, GuidanceRoutePreviewFragment::class.java, true)
        fragment.presenter.setDestination(entry)
    }

    override fun renderRoute(route: Route) {
        with(mapFragment!!) {
            renderRoute(route, false)
            zoomToBoundingBox(route.boundingBox)
        }
    }
