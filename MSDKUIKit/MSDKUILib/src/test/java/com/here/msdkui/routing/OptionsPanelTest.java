...
public class OptionsPanelTest extends RobolectricTest {
    ...
    public void testItemType() {
        final OptionItem booleanOptionItem = new OptionItemSpec.BooleanOptionItemBuilder(getApplicationContext()).setLabel(getString(R
                .string.msdkui_optimal)).build();
        mOptionPanel.setOptionsSpecs(Collections.singletonList(booleanOptionItem));
        assertThat(mOptionPanel.getOptionsSpecs().get(0), equalTo(booleanOptionItem));

        OptionItem optionItem = new OptionItemImpl(getContextWithTheme());
        mOptionPanel.setOptionsSpecs(optionItem, Collections.singletonList(booleanOptionItem));
        assertThat(mOptionPanel.getOptionsSpecs().get(0), equalTo(optionItem));
    }
