...
class OptionPanelFragmentTest : BaseTest() {
    ...
    public override fun setUp() {
        ...
        val controller = Robolectric.buildFragment(OptionPanelFragment::class.java).apply {
            get().routeOptions = mockRouteOptions
            get().dynamicPenalty = mockDynamicPenalty
        }
        `when`(mockRouteOptions.routeType).thenReturn(RouteOptions.Type.BALANCED)
        fragment = controller.create().get()
    ...
    fun testOnUiDataReady() {
        ...
        // checking count equal 5 because basic value is 3
        assertEquals(fragment.view.optionpanel_container.childCount, 5)
    }
    ...
    fun testUpdateOptions() {
        ...
        verify(presenter).updateOptions()
    }
