...
public class RouteDescriptionItem extends RelativeLayout {
    ...
    private void init(final Context context, final AttributeSet attrs, final int defStyleAttr, final int defStyleRes) {
        ...
        initSections();
        // set this item to invisible till mRoute is set.
    ...
    public void setRoute(final Route route) {
        ...
        mRoute = route;
        final RouteDescriptionHandler handler = new RouteDescriptionHandler(getContext(), mRoute);

        final ImageView icon = (ImageView) mSections.get(Section.TYPE_ICON);
        final Integer id = handler.getIcon();
        if (id == 0) {
        ...
        final Spannable totalTime = handler.getTimeToArrive(isTrafficEnabled());
        ...
        if (isTrafficEnabled() && !isBikeOrPedestrian()) {
            ...
            final Spannable delayText = handler.getTrafficDelayed();
        ...
        ((TextView) mSections.get(Section.DETAILS)).setText(handler.getDetails());
        ((SectionBar) mSections.get(Section.SECTION_BAR)).bind(handler.getSectionBar(), mSectionBarScaling);

        final String arrivalTime = handler.getArrivalTime(isTrafficEnabled());
    ...
    public void setSectionBarScaling(final float sectionBarScaling) {
        if (Float.compare(mSectionBarScaling, sectionBarScaling) != 0 && mRoute != null) {
            ((SectionBar) mSections.get(Section.SECTION_BAR)).bind(new RouteDescriptionHandler(getContext(), mRoute).
                    getSectionBar(), mSectionBarScaling);
        }
