...
/**
 * Tests for class {@link GuidanceSpeedLimitPanel}.
 */
public class GuidanceSpeedLimitPanelTest extends RobolectricTest {

    private static final int VELOCITY = 60;
    private static final int SPEED_LIMIT = 50;

    private GuidanceSpeedLimitPanel mSpeedLimitPanel = null;

    @Before
    public void setUp() {
        super.setUp();
        mSpeedLimitPanel = new GuidanceSpeedLimitPanel(getApplicationContext());
    }
    ...
    public void testViewDataIsParcelable() {
        final GuidanceSpeedData data = new GuidanceSpeedData(VELOCITY, SPEED_LIMIT);
        GuidanceSpeedLimitPanel.SavedState savedState = new GuidanceSpeedLimitPanel.SavedState(AbsSavedState.EMPTY_STATE);
        savedState.setStateToSave(data);
        Parcel parcel = Parcel.obtain();
        savedState.writeToParcel(parcel, savedState.describeContents());
        parcel.setDataPosition(0);

        GuidanceSpeedLimitPanel.SavedState createdFromParcel = GuidanceSpeedLimitPanel.SavedState.CREATOR.createFromParcel(
                parcel);
